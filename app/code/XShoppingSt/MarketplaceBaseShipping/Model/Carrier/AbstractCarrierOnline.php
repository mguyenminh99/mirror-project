<?php
namespace XShoppingSt\MarketplaceBaseShipping\Model\Carrier;
...
abstract class AbstractCarrierOnline extends CoreAbstractCarrierOnline
{
     ...
     * @param \XShoppingSt\Marketplace\Helper\Orders $marketplaceOrderHelper
     * @param \XShoppingSt\Marketplace\Model\ProductFactory $marketplaceProductFactory
     * @param \XShoppingSt\Marketplace\Model\SaleslistFactory $saleslistFactory
     * @param \XShoppingSt\MpAssignProduct\Model\ItemsFactory $assignItemsFactory
    ...
    public function __construct(
        ...
        \XShoppingSt\Marketplace\Helper\Orders $marketplaceOrderHelper,
        \XShoppingSt\Marketplace\Model\ProductFactory $marketplaceProductFactory,
    ...
    protected function isMultiShippingActive()
    {
        ...
        if ($routeName == 'multishipping' && ($moduleManager->isOutputEnabled("XShoppingSt_MpMultiShipping") &&
        $this->_scopeConfig->getValue('carriers/mpmultishipping/active'))) {
            return false;
        } elseif ($moduleManager->isOutputEnabled("XShoppingSt_MpMultiShipping") &&
        $this->_scopeConfig->getValue('carriers/mpmultishipping/active')) {
            return true;
        }
        return false;
    ...
    protected function isStorePickupActive()
    {
        $routeName = $this->requestInterface->getRouteName();
        $moduleManager = ObjectManager::getInstance()->create(\Magento\Framework\Module\Manager::class);
        if ($routeName == 'wkpickup' && ($moduleManager->isOutputEnabled("XShoppingSt_SellerStorePickup") &&
        $this->_scopeConfig->getValue('carriers/wkpickup/active'))) {
            return false;
        } elseif ($moduleManager->isOutputEnabled("XShoppingSt_SellerStorePickup") &&
        $this->_scopeConfig->getValue('carriers/wkpickup/active')) {
            return true;
        }
        return false;
    ...
    public function setRequest(\Magento\Quote\Model\Quote\Address\RateRequest $request)
    {
        ...
        foreach ($request->getAllItems() as $item) {
            ...
            $length=$item->getProduct()->getTsDimensionsLength();
            $height=$item->getProduct()->getTsDimensionsHeight();
            $width=$item->getProduct()->getTsDimensionsWidth();
